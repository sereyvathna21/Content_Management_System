...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!validateEmail(email)) { setError(t("errors.emailInvalid")); return; }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.message || t("errors.sendError"));
      } else {
        setIsSuccess(true);
      }
    } catch {
            ...
            {!isSuccess ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 animate-[fadeInUp_0.8s_ease-out_0.5s_both]">
                  <div className="animate-[slideInLeft_0.6s_ease-out_0.6s_both]">
                    ...
                    <input
                      ...
                      className={`block w-full text-gray-900 bg-white border-2 ${error ? "border-red-500" : "border-gray-300"} rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all duration-200 placeholder:text-gray-400 shadow-sm hover:shadow-md text-sm sm:text-base outline-none px-3 py-2.5 sm:px-4 sm:py-3`}
                    />
                    {error && <p id="email-error" role="alert" className="mt-1 text-red-500 text-xs sm:text-sm">{error}</p>}
                  </div>
                  ...
                  <div className="text-center animate-[fadeIn_0.6s_ease-out_0.8s_both]">
                    <Link href="/Authentication/Login" className="inline-flex items-center gap-2 text-white lg:text-primary hover:text-primary/80 transition-colors font-medium text-xs sm:text-sm">
              ...
              <div className="animate-[fadeIn_0.8s_ease-out]">
                ...
                <p className="lg:text-gray-600 text-white/90 text-center text-sm sm:text-base mb-6 sm:mb-8">
                  {t("success.message")}{" "}
                  <span className="font-semibold lg:text-gray-900 text-white">{email}</span>
                </p>
                <div className="space-y-4 sm:space-y-5">
                  <button onClick={() => { setIsSuccess(false); setEmail(""); }} className="w-full bg-primary text-white font-semibold rounded-xl hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 text-sm sm:text-base px-4 py-2.5 sm:px-6 sm:py-3">
                    {t("success.tryAnotherEmail")}
                  </button>
                  <Link href="/Authentication/Login" className="w-full inline-block text-center bg-white lg:text-primary text-primary border-2 border-primary font-semibold rounded-xl hover:bg-primary/5 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-all duration-200 shadow-sm hover:shadow-md text-sm sm:text-base px-4 py-2.5 sm:px-6 sm:py-3">
                    {t("success.backToSignIn")}
                  </Link>
                </div>
                <p className="lg:text-gray-500 text-white/70 text-center text-xs sm:text-sm mt-4 sm:mt-6">{t("success.didntReceive")}</p>
              </div>
            )}
          </div>
        </div>
