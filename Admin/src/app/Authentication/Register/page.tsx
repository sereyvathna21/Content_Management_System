...
export default function Register() {
  ...
  const validateForm = (): boolean => {
    const newErrors: FormErrors = { fullName: "", email: "", password: "", confirmPassword: "", agreeToTerms: "" };
    ...
    else if (!/[A-Z]/.test(formData.password) || !/[a-z]/.test(formData.password)) newErrors.password = t("errors.passwordUpperLower");
    else if (!/[0-9]/.test(formData.password)) newErrors.password = t("errors.passwordNumber");
    else if (!/[^A-Za-z0-9]/.test(formData.password)) newErrors.password = t("errors.passwordSpecial");
    ...
    <div>
      <section className="min-h-screen flex items-stretch">
        ...
        <div className="bg-white lg:w-1/2 w-full flex items-center justify-center z-0 relative border-t border-primary lg:border-l px-6 py-8 sm:px-8 sm:py-10 md:px-12 md:py-14 lg:px-16 lg:py-16">
          <div className="absolute lg:hidden inset-0 z-0 bg-cover bg-center" style={{ backgroundImage: "url(/login.svg)" }} />
          <div className="w-full max-w-md z-20">
            ...
            {registered ? (
              <div className="text-center mt-6 animate-[fadeIn_0.6s_ease-out_both]">
                ...
                <h2 className="font-bold text-white lg:text-gray-900 text-xl sm:text-2xl mb-2">{t("success.title")}</h2>
                <p className="text-white/90 lg:text-gray-600 text-sm sm:text-base mb-6">{t("success.message")}</p>
                <Link href="/Authentication/Login" className="inline-block w-full bg-primary text-white font-semibold rounded-xl text-center hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 text-sm sm:text-base px-4 py-2.5 sm:px-6 sm:py-3">
                ...
                <form onSubmit={handleSubmit} noValidate className="space-y-4 sm:space-y-5 animate-[fadeInUp_0.8s_ease-out_0.5s_both]">
                  {otpSent ? (
                    ...
                    <>
                      <div className="animate-[slideInLeft_0.6s_ease-out_0.55s_both]">
                        <label htmlFor="fullName" className="block font-medium lg:text-gray-700 text-white text-xs sm:text-sm mb-1.5 sm:mb-2">{t("fullNameLabel")}</label>
                        <input type="text" id="fullName" name="fullName" value={formData.fullName} onChange={handleChange} placeholder={t("fullNamePlaceholder")} autoComplete="name" aria-invalid={!!errors.fullName} className={inputClass(!!errors.fullName)} />
                        {errors.fullName && <p id="fullName-error" role="alert" className="mt-1 text-red-500 text-xs sm:text-sm">{errors.fullName}</p>}
                      </div>

                      <div className="animate-[slideInLeft_0.6s_ease-out_0.6s_both]">
                        <label htmlFor="email" className="block font-medium lg:text-gray-700 text-white text-xs sm:text-sm mb-1.5 sm:mb-2">{t("emailLabel")}</label>
                        <input type="email" id="email" name="email" value={formData.email} onChange={handleChange} placeholder={t("emailPlaceholder")} autoComplete="email" aria-invalid={!!errors.email} className={inputClass(!!errors.email)} />
                        {errors.email && <p id="email-error" role="alert" className="mt-1 text-red-500 text-xs sm:text-sm">{errors.email}</p>}
                      </div>

                      <div className="animate-[slideInLeft_0.6s_ease-out_0.65s_both]">
                        <label htmlFor="password" className="block font-medium lg:text-gray-700 text-white text-xs sm:text-sm mb-1.5 sm:mb-2">{t("passwordLabel")}</label>
                        <div className="relative">
                          <input type={showPassword ? "text" : "password"} id="password" name="password" value={formData.password} onChange={handleChange} placeholder={t("passwordPlaceholder")} autoComplete="new-password" aria-invalid={!!errors.password} className={`${inputClass(!!errors.password)} pr-10 sm:pr-12`} />
                          <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary rounded p-1 transition-colors" aria-label={showPassword ? t("hidePassword") : t("showPassword")}>
                        ...
                        {formData.password && (
                          <div className="mt-2">
                            <div className="flex gap-1 mb-1">
                              {[1, 2, 3, 4].map((step) => (
                                <div key={step} className={`flex-1 h-1.5 rounded-full transition-all duration-300 ${strength.score >= step ? strength.color : "bg-gray-200"}`} />
                              ))}
                            </div>
                            <p className="text-xs text-gray-500">{t(`strength.${strength.label}`)}</p>
                          </div>
                        )}
                      ...
                      <div className="animate-[slideInLeft_0.6s_ease-out_0.7s_both]">
                        ...
                        <div className="relative">
                          <input type={showConfirmPassword ? "text" : "password"} id="confirmPassword" name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} placeholder={t("confirmPasswordPlaceholder")} autoComplete="new-password" aria-invalid={!!errors.confirmPassword} className={`${inputClass(!!errors.confirmPassword)} pr-10 sm:pr-12`} />
                          <button type="button" onClick={() => setShowConfirmPassword((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary rounded p-1 transition-colors" aria-label={showConfirmPassword ? t("hidePassword") : t("showPassword")}>
                            <EyeIcon open={showConfirmPassword} />
                          </button>
                        </div>
                        {errors.confirmPassword && <p id="confirmPassword-error" role="alert" className="mt-1 text-red-500 text-xs sm:text-sm">{errors.confirmPassword}</p>}
                      </div>
                  ...
                  {!otpSent && (
                    <p className="text-center text-xs sm:text-sm lg:text-gray-600 text-white/90 animate-[fadeIn_0.6s_ease-out_0.9s_both]">
                      ...
                      <Link href="/Authentication/Login" className="font-semibold text-white lg:text-primary hover:text-primary/80 transition-colors">{t("signInLink")}</Link>
                    </p>
                  )}
                </form>
              </>
