...
export default React.memo(function VideoTable({
  ...
  const canEdit = can("video:update");
  const canDelete = can("video:delete");
  const canShowActions = canAny(["video:update", "video:delete"]);
  ...
  return (
    <>
    <div>
      {!loading && canCreate && onCreate && showInlineCreate && videos.length > 0 && (
                    ...
                    {videos.map((v) => {
                      return (
                        <TableRow key={v.id}>
                          <TableCell className="px-5 py-3 sm:px-6 text-start">
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <div className="font-medium line-clamp-2 text-left max-w-[250px] sm:max-w-[350px] md:max-w-[450px] lg:max-w-[600px] xl:max-w-[800px] whitespace-normal wrap-break-words">{v.title}</div>
                              </div>
                            </button>
                          </TableCell>
                          ...
                          <TableCell className="px-4 py-3 text-gray-500 text-start text-sm dark:text-gray-400">
                            {formatDate(v.publishAt, locale) ?? <span className="text-gray-300 italic">-</span>}
                          </TableCell>
                ...
                return (
                  <div key={v.id} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm space-y-3">
                    ...
                    <div className="text-xs text-gray-500">
                      Publish Date: {formatDate(v.publishAt, locale) ?? <span className="text-gray-300 italic">-</span>}
                    </div>
