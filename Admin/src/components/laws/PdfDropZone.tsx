...
export default function PdfDropZone({ file, onChange }: { file: File | null; onChange: (f: File | null) => void }) {
    ...
    >
      ...
      {file ? (
        ...
        <>
          ...
          <div>
            <p className="text-xs font-medium text-primary">{t("pdfUpload")} <span className="text-gray-400 font-normal">{t("pdfOrDrag")}</span></p>
            <p className="text-[10px] text-gray-400 mt-0.5">{t("pdfMaxSize")}</p>
          </div>
        </>
      )}
      {sizeError && (
        <p className="text-[11px] text-red-500 mt-1.5 z-20 relative">{t("pdfSizeError")}</p>
      )}
    </div>
  );
}
