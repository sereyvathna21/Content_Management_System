    ...
    <ComponentCard title="" className="mt-2">
      <div className="space-y-4">
        <div className="md:hidden space-y-3">
          {users.map((u) => (
            <div key={u.id} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm space-y-3">
              <button className="text-left w-full" onClick={() => onOpen(u)}>
              ...
              <div className="grid grid-cols-1 gap-2 text-sm text-gray-500">
                <div><span className="font-medium text-gray-700">{t("UserTable.headers.email")}: </span>{u.email}</div>
                <div><span className="font-medium text-gray-700">{t("UserTable.headers.password")}: </span>
                  <span className={`inline-block px-2 py-0.5 rounded text-theme-xs ${u.passwordSet ? "bg-green-100 text-green-700" : "bg-red-50 text-red-600"}`}>
                    {u.passwordSet ? t("UserTable.passwordSet") : t("UserTable.passwordNotSet")}
                  </span>
                </div>
                <div><span className="font-medium text-gray-700">{t("UserTable.headers.role")}: </span>{u.role}</div>
              </div>
