...
export default function UserForm({ open, onClose, onSave, initial }: Props) {
  ...
  useEffect(() => {
    if (initial) {
      ...
      setConfirmPassword("");
      setError(null);
      // if initial contains avatar, show it; otherwise show default avatar
      setAvatarPreview((initial as any).avatar || "/images/user/user-17.jpg");
    } else {
      setName("");
      setEmail("");
      setRole("editor");
      ...
      setConfirmPassword("");
      setError(null);
      setAvatarPreview("/images/user/user-17.jpg");
    }
  }, [initial, open]);
  ...
  function handleSubmit(e?: React.FormEvent) {
    ...
    onSave({ id: initial?.id, name, email, role, password: password || undefined, passwordSet: !!password, avatar: avatarPreview || (initial as any)?.avatar });
    onClose();
  }
  ...
  return (
    <Modal isOpen={open} onClose={onClose} className="max-w-xl p-6" backdropClassName="fixed inset-0 h-full w-full bg-gray-400/30 backdrop-blur-sm">
      <div className="mb-3">
        <h3 className="text-lg font-semibold text-primary">{initial ? "Edit User" : "Create User"}</h3>
        <p className="text-sm text-gray-500">Add or update user details below.</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
          ...
          <div className="md:col-span-2 space-y-3">
            ...
            <div>
              ...
              <PasswordInput placeholder="Password (leave blank to keep current)" value={password} onChange={(e) => setPassword(e.target.value)} />
